<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/Site.Master" CodeBehind="FacturaOld.aspx.vb" Inherits="TransAmets2.FacturaOld" %>
                                                                ...
                                                                <asp:GridView ID="grSelPortes" runat="server" 
				                                                    ...
				                                                    <Columns>
                                                                        ...
                                                                        <asp:TemplateField HeaderText="" ItemStyle-Width="6%" HeaderStyle-Width="6%" ItemStyle-HorizontalAlign="Center">
                                                                            <ItemTemplate>
                                                                                <asp:LinkButton ID="btnAdd_Porte" runat="server" usesubmitbehavior="true" CommandName="AddPorte" CommandArgument='<%# Bind("AUTOPORTE")%>'  Text="" ToolTip="Añadir Porte a Factura"><i class="fa fa-plus-square fa-2x"></i></asp:LinkButton>
...
<asp:Content ID="Content4" ContentPlaceHolderID="cphScripts" runat="server">
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            var numfac = $("#MainContent_hndAlta").val();
            if (numfac == "1") {
        ...
        function MuestraError(msg, tipo, titulo) {
            $('#MainContent_hdnControlJS').val(msg);
            $('#MainContent_hndTipoError').val(tipo);
            $('#MainContent_hdnTitulo').val(titulo);
            if (msg != '') {
                CompruebaErrores();
            }
        }
        $("#MainContent_btnGuardar").on('click', function () {
            if ($("#MainContent_hndBaja").val() == "1") {
        ...
        $('#CapaMensajes').on('hide.bs.modal', function (event) {
            $('#MainContent_hdnControlJS').val('');
            var EndSession = $('#MainContent_hndEndSession').val();
            if (EndSession == "S") {
                document.location.href = 'Login.aspx';
            }
        });
        $('#CapaMensajes').on('click', '#btnCapaMensajesAceptar', function (e) {
            var hdnRedirect = $("#MainContent_hdnRedirect").val();
            var hdnReload = $("#MainContent_hdnReload").val();
            if (hdnRedirect != "") {
        ...
        function ReloadDatepickers() {
            $("#MainContent_ddlTipoPorte").selectpicker('refresh');
            $("#MainContent_ddlTipoPorte").selectpicker('render');

            $("#MainContent_ddlAgencias").selectpicker('refresh');
            $("#MainContent_ddlAgencias").selectpicker('render');

            /* CALENDARIOS ============================================== */
            $('#MainContent_txtFecFactura').daterangepicker({
                ...
                $('#MainContent_txtFecFactura').val(start.format('DD/MM/YYYY'));
            });
            /* Acciones de calendarios para reiniciar los input */
            $('.btn-cleantxtFecFactura').on('click', function () {
                $("#MainContent_txtFecFactura").val('');
            });

            $('#MainContent_txtFecVencimiento').daterangepicker({
                "autoUpdateInput": false,
                "showDropdowns": true,
                "autoApply": false,
                "singleDatePicker": true,
                ...
                $('#MainContent_txtFecVencimiento').val(start.format('DD/MM/YYYY'));
            });
            /* Acciones de calendarios para reiniciar los input */
            $('.btn-cleantxtFecVencimiento').on('click', function () {
                $("#MainContent_txtFecVencimiento").val('');
            });

            $('#MainContent_txtFecCobro').daterangepicker({
                "autoUpdateInput": false,
                "showDropdowns": true,
                "autoApply": false,
                "singleDatePicker": true,
                ...
                $('#MainContent_txtFecCobro').val(start.format('DD/MM/YYYY'));
            });
            /* Acciones de calendarios para reiniciar los input */
            $('.btn-cleantxtFecCobro').on('click', function () {
                $("#MainContent_txtFecCobro").val('');
            });

            $('#MainContent_txtHastaFechaPortes').daterangepicker({
                "autoUpdateInput": false,
                "showDropdowns": true,
                "autoApply": false,
                "singleDatePicker": true,
                ...
                $('#MainContent_txtHastaFechaPortes').val(start.format('DD/MM/YYYY'));
            });
            /* Acciones de calendarios para reiniciar los input */
            $('.btn-cleantxtHastaFechaPortes').on('click', function () {
                $("#MainContent_txtHastaFechaPortes").val('');
            });
        };
        
        // SOLO NUMEROS sin puntos sin coma
        $("#MainContent_txtCliente").keydown(function (event) {
        ...
        $('#MainContent_txtObservaciones').keyup(function () {
            var obj = $('#MainContent_txtObservaciones');
            Calcula_NumCars(obj, 'observaciones_numcars', 1500)
        });
        $('#MainContent_txtFormaPago').keyup(function () {
            var obj = $('#MainContent_txtFormaPago');
